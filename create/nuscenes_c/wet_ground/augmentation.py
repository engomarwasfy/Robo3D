...
def estimate_laser_parameters(pointcloud_planes, calculated_indicent_angle, power_factor=15, noise_floor=0.7,
    ...
    # print('+++++++++:{}'.format(np.abs(np.max(normalized_intensitites))))
    ...
    hist, xedges, yedges = np.histogram2d(distance, normalized_intensitites, bins=(50, 2555),
                                          range=((10, 70), (0, np.abs(np.max(normalized_intensitites)))))  #Note it has been changed from 5 to 0
    idx = np.where(hist == 0)
...
if __name__ == '__main__':
    ...
    def _map(i: int) -> None:
        ...
        # ori_points = copy.deepcopy(points)
        ...
        # light: water_height=0.2mm, noise_floor=0.2; moderate: water_height=1.0mm, noise_floor=0.3; heavy: water_height=1.2mm, noise_floor=0.7;
