...
def process_single_channel(root_path: str, particle_file_prefix: str, orig_pc: np.ndarray, orig_label: np.ndarray, beam_divergence: float,
    ...
    # print(pc.shape)
    p_label = orig_label[channel_mask]
    ...
    return intensity_diff_sum, idx, pc, p_label
...
def augment(pc: np.ndarray, label: np.ndarray, particle_file_prefix: str, beam_divergence: float, shuffle: bool = True,
    ...
    if show_progressbar:
        ...
        channel_list[:] = process_map(functools.partial(process_single_channel, root_path, particle_file_prefix,
                                                        orig_pc,orig_label, beam_divergence, order, channel_infos),
        ...
        channel_list[:] = pool.map(functools.partial(process_single_channel, root_path, particle_file_prefix, orig_pc,orig_label,
    ...
    for item in channel_list:
        ...
        aug_pc[idx] = pc_
        aug_label[idx] = p_label
        snowflakes_sum += sum(p_label.reshape(-1) == 42)
    ...
    assert  len(aug_pc) == len(aug_label)
    # print(num_removed)
    # print("snowflakes_sum: {}".format(snowflakes_sum))
    # print(label.shape)
...
if __name__ == '__main__':
    ...
    for i,_ in enumerate(all_files):
        ...
        stats, augmented_pointcloud,augmented_label = augment(points, label, only_camera_fov=False,
        ...
        assert augmented_pointcloud.shape[1] == 5
        # print(augmented_pointcloud.shape)
        # augmented_pointcloud[:, 3] = np.clip(augmented_pointcloud[:,3] *255, 0, 255)
        ...
        augmented_label.astype(np.uint8).tofile(label_save_path)
