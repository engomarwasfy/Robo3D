...
def get_channel_info(points):
    """
    get channel info works for SemanticKITTI.
    """
    proj_fov_up =  3.0
    proj_fov_down = -25.0
    proj_H = 64
...
def augment(pc: np.ndarray, label: np.ndarray, particle_file_prefix: str, beam_divergence: float, shuffle: bool = True,
    ...
    # label = label[pc[:, 4].argsort()]       # sort label by channel
    ...
    for item in channel_list:
        ...
        snowflakes_sum += sum(p_label.reshape(-1) == 22)
...
if __name__ == '__main__':
    ...
    all_files = []
    all_files += absoluteFilePaths(
        '/'.join([src_folder, '08'.zfill(2), 'velodyne'])
    )
    all_files.sort()
